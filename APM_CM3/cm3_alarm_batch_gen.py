...
coil_ids = ['939500']
production_dates =['02/08/2022 21:29:00']
...
if row_count==0:
  ...
  spark.sql(f''' UPDATE daaentdevg2.pba_cpm_cm3_queue SET Status = "failed" WHERE Coil_ID IN ({coil_ids_str})''')
  dbutils.notebook.exit("Finished")

# COMMAND ----------

#coil_basic_schema = StructType([StructField("cg_Gen_Coilid", FloatType(), True), StructField("Min_Entry_Width", FloatType(), True),
#                                StructField("Max_Entry_Width", FloatType(), True), StructField("Alloy", FloatType(), True),  StructField("Production_date", StringType(), True)])
#coil_basic = spark.createDataFrame(spark.sparkContext.emptyRDD(),schema=coil_basic_schema)
#try:
...
#    basic_info_df = df.filter(df.cg_Gen_Coilid == float(coil_id)).groupBy('cg_Gen_Coilid').agg(min('cg_Gen_EntryWidth_PDI').alias('Min_Entry_Width'),max('pg_Gen_Max_EntryWidth').alias('Max_Entry_Width'),min('cg_Gen_Alloy').alias('Alloy'))
#    basic_info_df = basic_info_df.withColumn('Production_date',lit(production_date))
#    coil_basic = coil_basic.union(basic_info_df)
#except Exception as e:
#  failed_coils.append(coil_id)
#  coil_ids.remove(coil_id)
#  print('ERROR with ', coil_id,'\n', e)
#  pass
#  
## Write to Coil_Basic table
#DB = 'daaentdevg2'
#TABLENAME = 'pba_cpm_hfm_coil_basic'
#
##store data frame in transform folder as a delta table
#coil_basic.write.format("delta").mode("append").saveAsTable(DB+'.'+TABLENAME)

# COMMAND ----------

#d = rules['alarms']['al_hdr_speed_diff_sides']
#
#signals = []
#stack = list(d.items()) 
#visited = set()
...
#    k, v = stack.pop() 
#        
#    # if v is a dictionary, then flat it
...
#    if 'sig' in k:
#      if parent_key!='':
#        if [parent_key,k] not in signals:
#          signals.append([parent_key,k])
#        k1=''
#      else:
#        if [k] not in signals:
#          signals.append([k])
...
existent_signals = [x for x in cols_to_keep if x in df.columns] + ['cg_gen_coilid','eg_timeindex','dl_st1_wedgefactor','dl_excent_wedgefactor','dg_enwidth_width_meas','dl_incooltable_widthadjustment_a_meas','dl_incooltable_widthadjustment_b_meas','eg_st2_millstatus']
missing_signals = [x for x in cols_to_keep if x not in df.columns]
...
spark.sql(f'''OPTIMIZE {STATUS_TABLE} ZORDER BY (Coil_ID)''')

# COMMAND ----------

DB ='daaentdevg2'
QUEUE_TABLE = 'pba_cpm_cm3_queue'
COILS = str(coil_ids)[1:-1]

try:
  query = f''' UPDATE {DB}.{QUEUE_TABLE} SET Status = "processed" WHERE Coil_ID IN ({COILS})'''
  spark.sql(query)#.format(str(coil_ids)[1:-1]))
  query2= f''' OPTIMIZE {DB}.{QUEUE_TABLE}'''
  print(query)
except Exception as e:
  print(e)
  pass
